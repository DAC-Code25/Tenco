...
首页手动速度直接经独立 WebSocket 通道发送；自动必须在地图任务页明确启动。已移除旧模式寄存器写入和 GPS 文件上传。原点更新、任务下发、位姿及拍摄恢复的当前接口见 [工控机闭环操作与接口](工控机闭环操作与接口.md)。
...
手动和自动使用不同连接，均沿用 cmd_vel/isRemote=true；不要求控制权回包，也不发送脚本启动或 stopLocation。IPC 实测速度来自融合编码器，连接状态与实测速度新鲜度分别检查。详见上述接口文档。
